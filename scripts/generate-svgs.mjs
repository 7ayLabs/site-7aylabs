/**
 * Generate themed SVG icon files for 7aychain landing sections.
 * Creates dark/ and light/ variants with explicit accent colors.
 *
 * Run: node scripts/generate-svgs.mjs
 */
...
function iconConnect(c) {
  ...
  <path d="M40 22C44 18 44 14 40 10" stroke="${c}" stroke-width="2" stroke-linecap="${CAP}" fill="none" opacity="0.7"/>
  <path d="M44 26C50 20 50 12 44 6" stroke="${c}" stroke-width="1.5" stroke-linecap="${CAP}" fill="none" opacity="0.45"/>
</svg>`;
}
...
function iconPresence(c) {
  ...
  <path d="M20 48C20 38 25 33 32 33C39 33 44 38 44 48" stroke="${c}" stroke-width="${SW}" stroke-linecap="${CAP}" fill="none"/>
...
function iconLocation(c) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64" fill="none">
  <path d="M32 56C32 56 12 38 12 24C12 13 21 4 32 4C43 4 52 13 52 24C52 38 32 56 32 56Z" stroke="${c}" stroke-width="${SW}" stroke-linejoin="${JOIN}" fill="none"/>
...
function iconPrivacy(c) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64" fill="none">
  <path d="M6 32C6 32 16 18 32 18C48 18 58 32 58 32C58 32 48 46 32 46C16 46 6 32 6 32Z" stroke="${c}" stroke-width="${SW}" stroke-linejoin="${JOIN}" fill="none"/>
...
function iconCommunity(c) {
  ...
  <path d="M22 42C22 34 26 30 32 30C38 30 42 34 42 42" stroke="${c}" stroke-width="${SW}" stroke-linecap="${CAP}" fill="none"/>
  <circle cx="14" cy="24" r="4" stroke="${c}" stroke-width="1.8" fill="none" opacity="0.6"/>
  <path d="M6 44C6 38 9 34 14 34C17 34 19 35 21 37" stroke="${c}" stroke-width="1.8" stroke-linecap="${CAP}" fill="none" opacity="0.6"/>
  <circle cx="50" cy="24" r="4" stroke="${c}" stroke-width="1.8" fill="none" opacity="0.6"/>
  <path d="M58 44C58 38 55 34 50 34C47 34 45 35 43 37" stroke="${c}" stroke-width="1.8" stroke-linecap="${CAP}" fill="none" opacity="0.6"/>
</svg>`;
}
...
console.log("Generating themed SVG icons...\n");

for (const icon of ICONS) {
  const pal = PALETTE[icon.palette];

  const darkSvg = icon.fn(pal.dark);
  writeFileSync(join(OUT_DARK, `${icon.name}.svg`), darkSvg);
  console.log(`  dark/${icon.name}.svg`);

  const lightSvg = icon.fn(pal.light);
  writeFileSync(join(OUT_LIGHT, `${icon.name}.svg`), lightSvg);
  console.log(`  light/${icon.name}.svg`);
}

console.log(`\nDone! Generated ${ICONS.length * 2} SVG icons.`);
